...
function withCORS(headers, request) {
    headers['access-control-allow-origin'] = 'https://blooket-utility.herokuapp.com';
...
function getHandler(options, proxy) {
    ...
    return function (req, res) {
        ...
        try {
            if (req.headers['bu-cookie']) req.headers.cookie = req.headers['bu-cookie'];
        } catch {}
...
const createServer = function createServer(options) {
    ...
    proxy.on('error', function proxyErrorHandler(err, req, res) {
        ...
        res.writeHead(404, {'Access-Control-Allow-Origin': 'https://blooket-utility.herokuapp.com'});
