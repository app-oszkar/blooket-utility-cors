...
function withCORS(headers, request) {
  headers["access-control-allow-origin"] =
    "https://blooket-utility.okr765.com";
  headers["access-control-allow-credentials"] = "true";
  if (headers["set-cookie"]) {
    headers["bu-set-cookie"] = headers["set-cookie"][0];
  }
...
function proxyRequest(req, res, proxy) {
  ...
  const proxyThroughUrl = req.corsEscapeRequestState.getProxyForUrl(
    location.href
  );
  if (proxyThroughUrl) {
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  if ([301, 302, 303, 307, 308].includes(statusCode)) {
    ...
    if (locationHeader) {
      ...
      debug(
        `Request to ${requestState.location.href} redirecting with status ${statusCode} to ${locationHeader}`
      );
...
function getHandler(options, proxy) {
  ...
  if (corsEscape.requireHeaders.length) {
    corsEscape.requireHeaders = corsEscape.requireHeaders.map((header) =>
      header.toLowerCase()
    );
  }
  const hasRequiredHeaders = (providedHeaders) => {
    return (
      corsEscape.requireHeaders.length === 0 ||
  ...
  return function (req, res) {
    ...
    const rateLimitMessage =
      corsEscape.checkRateLimit && corsEscape.checkRateLimit(origin);
    if (rateLimitMessage) {
    ...
    try {
      if (req.headers["bu-cookie"])
        req.headers.cookie = req.headers["bu-cookie"];
    } catch {}
...
const createServer = function createServer(options) {
  ...
  proxy.on("error", function proxyErrorHandler(err, req, res) {
    ...
    res.writeHead(404, {
      "Access-Control-Allow-Origin": "https://blooket-utility-cors.vercel.app",
    });
